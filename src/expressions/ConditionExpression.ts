...
export type AttributeType = "S" | "SS" | "N" | "D" | "NS" | "DS" | "B" | "BS" | "BOOL" | "NULL" | "L" | "M";
...
export interface SizePredicate extends BaseFunctionExpressionPredicate {
  type: "Function";
  name: "size";
...
export type FunctionExpressionPredicate = AttributeExistsPredicate | AttributeNotExistsPredicate | AttributeTypePredicate | BeginsWithPredicate | ContainsPredicate | SizePredicate;
...
export function isConditionExpressionSubject(arg: any): arg is ConditionExpressionSubject {
  return Boolean(arg) && typeof arg === "object" && (typeof arg.subject === "string" || AttributePath.isAttributePath(arg.subject));
}

export type SimpleConditionExpression = ConditionExpressionSubject & ConditionExpressionPredicate;

export type ConditionExpression = SimpleConditionExpression | AndExpression | OrExpression | NotExpression | FunctionExpression;
...
export function serializeConditionExpression(condition: ConditionExpression, attributes: ExpressionAttributes): string {
  ...
  switch (condition.type) {
    ...
    case "GreaterThan":
    case "GreaterThanOrEqualTo":
      const operator = comparators[condition.type];
      return serializeBinaryComparison(condition, attributes, operator);
    case "Between":
    ...
    case "Membership":
      return `${attributes.addName(condition.subject)} IN (${condition.values.map((val) => serializeOperand(val, attributes)).join(", ")})`;
    case "Function":
      const subject = AttributePath.isAttributePath(condition.subject) ? condition.subject : new AttributePath(condition.subject);
      ...
      switch (condition.name) {
        ...
        case "size": {
          const subjectSize = new FunctionExpression(condition.name, subject).serialize(attributes);
          const comparedTo = serializeSizeOperator(condition.condition, attributes);
          return `${subjectSize} ${comparedTo}`;
        }
      }
    case "Not":
    ...
    case "Or":
      ...
      return condition.conditions.map((cond) => `(${serializeConditionExpression(cond, attributes)})`).join(` ${condition.type.toUpperCase()} `);
  }
}

function serializeBinaryComparison(cond: BinaryComparisonPredicate & ConditionExpressionSubject, attributes: ExpressionAttributes, comparator: string): string {
...
function serializeSizeOperator(
  condition:
    | EqualityExpressionPredicate
    | InequalityExpressionPredicate
    | LessThanExpressionPredicate
  ...
  switch (condition.type) {
    case "Membership":
      return `IN (${condition.values.map((x) => attributes.addValue(x)).join(", ")})`;
    case "Between":
      return `BETWEEN ${attributes.addValue(condition.lowerBound)} AND ${attributes.addValue(condition.upperBound)}`;
    default:
      const comparator = comparators[condition.type];
      return `${comparator} ${attributes.addValue(condition.object)}`;
  }
}

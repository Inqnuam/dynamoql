  ...
  for (let iter = path[Symbol.iterator](), curr = iter.next(), peek = iter.next(); curr.done === false; curr = peek, peek = iter.next()) {
    if (state === ParseState.identifier) {
      switch (curr.value) {
        ...
        case PATH_DELIMITER:
          if (collected === "") {
            throw new DynamoQLException(`Invalid control character encountered in path: ${path}`);
          }
      ...
      switch (curr.value) {
        case RIGHT_BRACKET:
          ...
          if (!isFinite(intVal)) {
            throw new DynamoQLException(`Invalid array index (${collected}) encountered in path: ${path}`);
          }
        ...
        case "9":
          ...
          break;
        default:
          throw new DynamoQLException(`Invalid array index character (${curr.value}) encountered in path: ${path}`);
      }
    } else {
      switch (curr.value) {
        case LEFT_BRACKET:
        ...
        case PATH_DELIMITER:
          ...
          break;
        default:
          throw new DynamoQLException(`Bare identifier encountered between list index accesses in path: ${path}`);
      }
    }
  }

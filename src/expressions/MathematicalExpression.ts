...
export class MathematicalExpression implements AttributeBearingExpression {
  ...
  constructor(readonly lhs: MathematicalExpressionOperand, readonly operator: "+" | "-", readonly rhs: MathematicalExpressionOperand) {}
  ...
  serialize(attributes: ExpressionAttributes) {
    const safeArgs = [this.lhs, this.rhs].map((arg) => {
      ...
      return AttributePath.isAttributePath(arg) || typeof arg === "string" ? attributes.addName(arg) : attributes.addValue(arg);
    });

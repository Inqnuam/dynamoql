    ...
    for (const [key, value] of this.toSet.entries()) {
      phrases.push(
        `${attributes.addName(key)} = ${
          FunctionExpression.isFunctionExpression(value) || MathematicalExpression.isMathematicalExpression(value) ? value.serialize(attributes) : attributes.addValue(value)
        }`
      );
    }
    if (phrases.length > 0) {
